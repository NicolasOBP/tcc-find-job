...
export default function CadProfe() {
  ...
  async function cad(ev) {
    ev.preventDefault();
    const isValid = /^[A-Za-zÀ-ÖØ-öø-ÿ ]+$/.test(nomeP);
    if (
      nomeP == "" ||
      emailP == "" ||
      senhaP == "" ||
      ...
      setContModal("Não pode conter número no nome");
      setShowBtn(true);
      setModal(true);
    } else if (senhaP.length < 7) {
      setContModal("Senha é muito curta");
      setShowBtn(true);
      setModal(true);
    } else if (senhaP == confsenhaP) {
      try {
            ...
            <div className="sm:col-span-1">
              ...
              <div className="mt-2">
                <InputMask
                  ...
                  value={telP}
                  onChange={(ev) => setTelP(ev.target.value)}
                  mask="(99) 99999-9999"
            ...
            <div className="sm:col-span-1">
              ...
              <div className="mt-2">
                <InputMask
                  className="block w-full rounded-md border-0 py-1.5 px-1.5 text-black-900 shadow-sm ring-2 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
                  value={cpfP}
                  onChange={(ev) => setCpfP(ev.target.value)}
                  mask="999.999.999-99"
