...
export default function NavbarE(props) {
  ...
  const navigation = [
    {
      name: "Seleção de Candidato",
      href: "/candidatos-empresa",
      current: true,
    },
    { name: "Suas vagas", href: "/suas-vagas", current: false },
    { name: "Cadastrar nova vaga", href: "/candastro-vagas", current: false },
  ];
              ...
              {props.svaga ? (
                <div className="flex absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0 space-between">
                  <button
                    ...
                    className="rounded-lg p-1 font-semibold text-md text-purple"
                  >
                    {dados.logo ? (
                      <img
                ...
                <div className="flex absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0 space-between">
                  <button
                    className="rounded-lg bg-red-500 p-1 font-semibold text-md text-white border shadow-md"
                    onClick={logout}
                  >
                    Encerrar sessão
                  </button>
                            ...
                            <Menu.Item key={i++}>
                              <>
                                <input
                                  type="checkbox"
                                  value={v}
                                  checked={selecFilter.includes(v)}
                                ...
                                />
                                <label className="px-4 py-2 text-sm text-gray-700">
                                  {v}
                                </label>
                              </>
                            </Menu.Item>
